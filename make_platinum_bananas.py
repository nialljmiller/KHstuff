'''
make_platinum_bananas.py

Computes age-metallicity banana curves for each platinum sample star using
MCMC (emcee) with a constrained 3D log-probability function.
...
At every walker step, the constrained parameters are derived from initial_met:
    alpha_fe      = 0.0                        (fixed: bulge run assumption)
...
Y relation) — the full degenerate banana surface — rather than 28 discrete
optimizer results. Boundary hits are impossible by construction: walkers that
wander outside the grid return -inf log-probability.

HPC usage (one job per star):
    python make_platinum_bananas.py --star_id 2M17443488-2901000
    python make_platinum_bananas.py --star_index 0

Run all stars sequentially (local / single node):
    python make_platinum_bananas.py

After all stars are done:
    python make_platinum_bananas.py --combine

Outputs (per star)
------------------
results/bananas/chains/<star_id>.pkl
    Dict with keys: chain (n_walkers×n_iter×3), flat_samples (DataFrame),
    blobs (DataFrame of interpolated stellar params), star metadata.

results/bananas/plots/<star_id>.png
    Banana plot from MCMC samples.

Outputs (combined, after --combine)
------------------------------------
results/bananas/banana_data.csv
    Long-format: one row per MCMC sample, all stars.

results/bananas/bananas.pkl
    Dict: star_id → DataFrame of flat samples + derived quantities.
    Direct input for step 4 (MDF sampling).

results/bananas/banana_summary.png
    RGB vs clump banana summary.
'''
...
# ── Output directories ────────────────────────────────────────────────────────
os.makedirs('results/bananas/chains', exist_ok=True)
os.makedirs('results/bananas/plots',  exist_ok=True)

# ── MCMC settings ─────────────────────────────────────────────────────────────
N_WALKERS  = 32    # must be even and > 2 * ndim (ndim=3, so min=8; 32 is comfortable)
N_BURNIN   = 300   # burn-in steps (discarded)
N_ITER     = 1000  # production steps per walker → 32000 samples per star
NDIM       = 3     # free dimensions: initial_mass, initial_met, eep

# Observational uncertainties for the Gaussian likelihood
# teff: use per-star e_teff from catalogue
# lum: FLAME luminosities don't have formal per-star errors; use 0.1 dex
...
def make_initial_positions(star_row, bounds, n_walkers, rng):
    '''
    Scatter walkers around the gridsearch_fit result from the platinum run.
    Falls back to grid centre if no fit result is available.
    '''
    mass_lo, mass_hi = bounds['initial_mass']
    met_lo,  met_hi  = bounds['initial_met']
    eep_lo,  eep_hi  = bounds['eep']

    # Use previous fit as centre if available
    ...
    if np.isnan(c_mass): c_mass = (mass_lo + mass_hi) / 2
    if np.isnan(c_met):  c_met  = (met_lo  + met_hi)  / 2
    if np.isnan(c_eep):  c_eep  = (eep_lo  + eep_hi)  / 2

    # Scatter widths: ~5% of range
...
def save_banana_plot(star_id, flat_samples, blobs_df,
    ...
    fig.suptitle(
        ...
        f"obs[M/H]={mh_obs:.2f}   platinum_loss={orig_loss:.3f}   "
        f"N_samples={n_eff:,}",
        fontsize=10, fontweight='bold'
    )
    fig.tight_layout()
    fig.savefig(f'results/bananas/plots/{safe_id}.png', dpi=110, bbox_inches='tight')
    plt.close(fig)
...
def run_star(star_row, jtgrid, bounds):
    ...
    orig_loss     = float(star_row['fit_loss']) if not np.isnan(
    ...
    # ── Extract chain ─────────────────────────────────────────────────────────
    flat_chain = sampler.flatchain   # (N_WALKERS*N_ITER, 3)
    flat_samples = pd.DataFrame(flat_chain,
                                columns=['initial_mass', 'initial_met', 'eep'])

    # Add derived constrained parameters
    ...
    # ── Extract blobs (interpolated stellar params) ───────────────────────────
    blob_list = []
    ...
    chain_path = f'results/bananas/chains/{safe_id}.pkl'
...
def combine_chains(fit_df):
    ...
    print("Saved to results/bananas/banana_data.csv")

    with open('results/bananas/bananas.pkl', 'wb') as f:
        pickle.dump(banana_dict, f)
    print("Banana dict saved to results/bananas/bananas.pkl")

    # ── Summary plot ─────────────────────────────────────────────────────────
    print("Making summary banana plot...")
    fig, axes = plt.subplots(1, 2, figsize=(14, 6), sharey=True)
    fig.suptitle(
        "Platinum Sample — Age–[Fe/H] Bananas (MCMC)\n"
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='Platinum banana MCMC')
    ...
    if args.star_id is not None:
        ...
        # Run all stars sequentially
        already_done = set(
            f.replace('.pkl', '')
            for f in os.listdir('results/bananas/chains')
            if f.endswith('.pkl')
        )
