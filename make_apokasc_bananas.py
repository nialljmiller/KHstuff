...
Runs the same age-metallicity banana MCMC pipeline used for the platinum
bulge sample, but on APOKASC stars where asteroseismic ages are known.

Purpose: validation. If the method recovers IntAge for APOKASC stars, the
inference is not the source of discrepancies seen in the bulge sample.

Input:  MeridithRomanApokascCalibLtest5ns3L.out
Output: results/apokasc/chains/<star_id>.pkl  (same format as platinum chains)
        results/apokasc/banana_grid.png
        results/apokasc/age_comparison.png/.pdf

Usage:
    python make_apokasc_bananas.py                      # run all stars
    python make_apokasc_bananas.py --star_index 0       # single star by index
    python make_apokasc_bananas.py --combine            # combine + plot

Luminosity is computed from Teff, Logg and IntMass using:
    log(L/Lsun) = log(M/Msun) + 4*log(Teff/5777) - (logg - 4.4374)
Stars with IntMass = -99 or IntAge = -99 are skipped.
'''
...
def run_star(star_row, jtgrid, bounds):
    ...
    sampler = emcee.EnsembleSampler(
        ...
        kwargs=dict(
            interp=jtgrid,
            teff_obs=teff_obs, lum_obs=lum_obs,
            teff_sigma=e_teff, lum_sigma=LUM_SIGMA_DEFAULT,
            bounds=bounds, ml_lo=ml_lo, ml_hi=ml_hi,
        )
    )

    sampler.run_mcmc(p0, N_BURNIN + N_ITER, progress=False)
    flat_samples = sampler.get_chain(discard=N_BURNIN, flat=True)
    flat_blobs   = sampler.get_blobs(discard=N_BURNIN, flat=True)
    acc          = float(np.mean(sampler.acceptance_fraction))

    blob_rows = [b for b in flat_blobs if b is not None]
    blobs_df  = pd.DataFrame(blob_rows) if blob_rows else pd.DataFrame()

    age_col = 'age' if ('age' in blobs_df.columns) else 'Age(Gyr)'
    cols_keep = ['teff', 'lum', age_col]
    cols_keep = [c for c in cols_keep if c in blobs_df.columns]

    output = pd.DataFrame(flat_samples, columns=['initial_mass', 'initial_met', 'eep'])
    for col in cols_keep:
        output[col] = blobs_df[col].values[:len(output)]
...
def combine_and_plot():
    ...
    # 1 — Our age vs asteroseismic age
    fig, ax = plt.subplots(figsize=(5.5, 5.5))
    for i in range(len(our_ages)):
        ax.errorbar(seismic_ages[i], our_ages[i],
    ...
    # 2 — Residual vs seismic age
    residuals = our_ages - seismic_ages
    fig, ax = plt.subplots(figsize=(5.5, 4.5))
    for i in range(len(our_ages)):
        ax.errorbar(seismic_ages[i], residuals[i],
                    yerr=[[our_lo[i]], [our_hi[i]]],
                    fmt='o', color=cols[i], ms=5, lw=1.0,
                    capsize=2.0, ecolor=cols[i], zorder=3)
...
if __name__ == '__main__':
    ...
    if args.star_index is not None:
        if args.star_index >= len(stars):
            ...
            sys.exit(1)
        run_star(stars.iloc[args.star_index], jtgrid, bounds)
    else:
        n = len(stars)
        for i, (_, row) in enumerate(stars.iterrows()):
            print(f"[{i+1:4d}/{n}]", end=' ')
            run_star(row, jtgrid, bounds)
